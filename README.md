...
**MoonBit WAV 音频工具库 — 效果器、波形生成与格式处理**

A MoonBit WAV audio toolkit with effects, waveform generation, and format conversion.

一个用 MoonBit 实现的 WAV 音频工具库，侧重音频效果处理、波形信号生成和格式转换。

> **与 [RidgeLab/moonwavkit](https://github.com/Ridge-Lab/MoonWavKit) 的关系：** moonwavkit 是 MoonBit 生态中成熟的 WAV 解析与 PCM 分析库，侧重结构解析、元数据提取、质量报告和 fixture 验收。moon-wav 在此基础上关注不同的侧重点——音频效果器（回声、滤波、软限幅等）、波形信号生成（正弦/方波/锯齿/噪声等）和位深转换。如果你需要 WAV 底层解析和质量检查，推荐优先使用 moonwavkit；如果你需要效果处理和信号生成，moon-wav 可以作为补充。
...
- **音频效果** - 回声、延迟、归一化、软限幅、低通/高通滤波、反向混响
